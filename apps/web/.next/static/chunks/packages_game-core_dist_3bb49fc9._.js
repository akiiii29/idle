...
"[project]/packages/game-core/dist/constants/beasts.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/constants/config.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/constants/pet-config.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/constants/accessory-config.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
const PCT = (x)=>"".concat((x * 100).toFixed(x >= 0.1 && x < 10 ? 0 : 1), "%");
function describeAccessoryEffect(eff) {
    let upgradeLevel = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 0;
    const scale = 1 + upgradeLevel * 0.05;
    const p = eff.power * scale;
    switch(eff.type){
        case "CRIT_CHANCE":
            return "Tỉ lệ **chí mạng** +".concat(PCT(p), " (cộng vào xác suất crit trong trận)");
        case "CRIT_DMG":
            return "**Sát thương khi chí mạng** (hệ số nhân) +".concat(PCT(p), " — cộng dồn lên nền 1.5");
        case "BURN_DMG":
            return "**Sát thương Đốt** (hệ số nhân DOT) +".concat(PCT(p));
        case "BURN_DUR":
            return "**Thời lượng / cường độ đốt** +".concat(eff.power * scale, " (theo hệ combat)");
        case "POISON_DMG":
            return "**Sát thương Độc** (hệ số nhân DOT) +".concat(PCT(p));
        case "POISON_DUR":
            return "**Thời lượng / cường độ độc** +".concat(Math.floor(eff.power * scale), " (theo hệ combat)");
        case "LIFESTEAL":
            return "**Hút máu** (hệ số nhân) +".concat(PCT(p), " — hồi HP theo % ST gây ra");
        case "MULTI_HIT":
            return "**Đánh kép / đa đòn** — cơ hội gây thêm đòn (power ".concat(p.toFixed(2), ")");
        case "REDUCE_DMG":
            return "**Giảm sát thương nhận** (hệ số defenseMult) +".concat(PCT(p));
        case "PROC_CHANCE":
            return "**Tỉ lệ kích hoạt skill** (proc) +".concat(PCT(p));
        case "LOW_HP_DMG":
            return "**Sát thương tăng khi máu thấp** (power ".concat(p.toFixed(2), ")");
        case "PET_DMG":
            return "**Sát thương từ pet** +".concat(PCT(p));
        case "PET_CHANCE":
            return "**Tỉ lệ pet kích hoạt** +".concat(PCT(p));
        case "PET_PLAYER_SYN":
            return "**Đồng bộ pet ↔ nhân vật** (synergy, power ".concat(p.toFixed(2), ")");
        case "HYBRID_BURN_POISON":
            return "**Cộng hưởng Đốt + Độc** (hybrid, power ".concat(p.toFixed(2), ")");
        case "UNIQUE_CRIT_EXECUTE":
            return "**Nội tại:** ST chí mạng thêm **+".concat(PCT(p), "** khi kẻ địch dưới **50%** máu (theo combat)");
        case "UNIQUE_BURN_INSTANT":
            return "**Nội tại:** đốt có thể gây **bùng nổ / tick tức thì** (power ".concat(p.toFixed(2), ")");
        case "UNIQUE_POISON_BURST":
            return "**Nội tại:** cơ chế **nổ độc** / burst poison khi đủ điều kiện";
        case "UNIQUE_LIFE_DOUBLE":
            return "**Nội tại:** tăng hiệu quả **hút máu** khi máu thấp (power ".concat(p.toFixed(2), ")");
        case "UNIQUE_BLOCK_HIT":
            return "**Nội tại:** có cơ hội **chặn hoàn toàn** một đòn (theo lượt)";
        case "UNIQUE_PROC_TWICE":
            return "**Nội tại:** skill có thể **kích hoạt kép** / proc nhân (power ".concat(p.toFixed(2), ")");
        case "UNIQUE_BERSERK_PERCENT":
            return "**Nội tại:** ST tăng theo **% máu đã mất** (hệ số ".concat(p.toFixed(3), " × 100 mỗi 1% máu thiếu — xem combat)");
        default:
            return "".concat(eff.type, " (power ").concat(p.toFixed(3), ")");
    }
}
function describeSetBonusEffects(effects, label) {
    if (!effects.length) return "";
    const lines = effects.map((e)=>"  · ".concat(describeAccessoryEffect(e, 0)));
    return "**".concat(label, "**\n").concat(lines.join("\n"));
}
function describeAccessoryForShop(config) {
    const lines = [];
    lines.push("📦 **Bộ:** ".concat(config.set));
    lines.push("");
    lines.push("**Hiệu ứng món này (+0):**");
    for (const e of config.effects){
        lines.push("• ".concat(describeAccessoryEffect(e, 0)));
    }
...
"[project]/packages/game-core/dist/constants/item-pool.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
 ...
 *  Lower floors → more COMMON. Higher floors → chance for EPIC/LEGENDARY. */ function rollItemRarity(floor) {
    let luckBonus = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 0;
...
/** Roll a random floor-appropriate item drop (weapon or armor 50/50) */ function rollLootDrop(floor) {
    let luckBonus = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 0;
...
"[project]/packages/game-core/dist/constants/relic-pool.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/constants/titles.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/constants/synergy-hints.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
const pet_config_1 = __turbopack_context__.r("[project]/packages/game-core/dist/constants/pet-config.js [app-client] (ecmascript)");
...
"[project]/packages/game-core/dist/constants/index.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/constants/beasts.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/constants/config.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/constants/pet-config.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/constants/accessory-config.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/constants/item-pool.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/constants/relic-pool.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/constants/titles.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/constants/synergy-hints.js [app-client] (ecmascript)"), exports);
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
}),
"[project]/packages/game-core/dist/types/combat.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/types/rpg-enums.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/types/index.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/types/combat.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/types/rpg-enums.js [app-client] (ecmascript)"), exports);
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
}),
"[project]/packages/game-core/dist/services/rng.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/services/combat-utils.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
 ...
 */ function applyDamage(target, damage, events) {
    if (target.shield > 0) {
        ...
        events.push("🛡️ **Giáp ảo** hấp thụ **".concat(absorbed, "**! (Còn lại: ").concat(Math.floor(target.shield), ")"));
        if (remainingDmg > 0) {
            target.hp -= remainingDmg;
            events.push("🩸 **".concat(target.name, "** chịu thêm **").concat(Math.floor(remainingDmg), "** sát thương vào HP."));
        }
    } else {
        target.hp -= damage;
        events.push("🩸 **".concat(target.name, "** chịu **").concat(Math.floor(damage), "** sát thương."));
    }
 ...
 */ function addEffect(effects, newEffect) {
    let maxStacks = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 5;
    ...
    for (const e of effects){
        ...
        switch(e.type){
            case "hot":
                ...
                events.push("🟢 **".concat(e.name, "** hồi phục **").concat(Math.floor(heal), "** HP."));
                break;
            case "dot":
            ...
            case "poison":
                ...
                events.push("🤢 **Độc tố** (x".concat(e.stacks, ") gây thêm áp lực."));
                break;
            case "burn":
 ...
 */ function cleanupEffects(effects, events) {
    ...
    for(let i = effects.length - 1; i >= 0; i--){
        ...
        e.turns--;
        if (e.turns <= 0) {
            events.push("⏳ Hiệu ứng **".concat(e.name, "** đã hết tác dụng."));
            effects.splice(i, 1);
        }
    }
}
...
"[project]/packages/game-core/dist/services/leveling.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
function applyLevelUps(stats) {
    ...
    while(updated.exp >= requiredExpForLevel(updated.level)){
        ...
        summaries.push("Level ".concat(updated.level, ": STR +").concat(strGain, ", AGI +").concat(agiGain).concat(gainedLuck > 0 ? ", LUCK +1" : "", ", MAX HP +").concat(hpGain));
    }
    return {
        updated,
        levelsGained,
...
"[project]/packages/game-core/dist/services/hunt-service.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
const beasts_1 = __turbopack_context__.r("[project]/packages/game-core/dist/constants/beasts.js [app-client] (ecmascript)");
const pet_config_1 = __turbopack_context__.r("[project]/packages/game-core/dist/constants/pet-config.js [app-client] (ecmascript)");
const config_1 = __turbopack_context__.r("[project]/packages/game-core/dist/constants/config.js [app-client] (ecmascript)");
const rng_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/rng.js [app-client] (ecmascript)");
...
function createWildBeast(level, luck) {
    ...
    return {
        ...
        rarity,
        power,
        role: config === null || config === void 0 ? void 0 : config.role,
        skillType: config === null || config === void 0 ? void 0 : config.skillType,
        skillPower: config === null || config === void 0 ? void 0 : config.skillPower,
        trigger: config === null || config === void 0 ? void 0 : config.trigger
    };
}
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
...
"[project]/packages/game-core/dist/services/pet-utils.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
function enrichBeast(beast) {
    ...
    var _beast_role, _beast_skillType, _beast_skillPower, _beast_trigger;
    return {
        ...beast,
        role: (_beast_role = beast.role) !== null && _beast_role !== void 0 ? _beast_role : config.role,
        skillType: (_beast_skillType = beast.skillType) !== null && _beast_skillType !== void 0 ? _beast_skillType : config.skillType,
        skillPower: (_beast_skillPower = beast.skillPower) !== null && _beast_skillPower !== void 0 ? _beast_skillPower : config.skillPower,
        trigger: (_beast_trigger = beast.trigger) !== null && _beast_trigger !== void 0 ? _beast_trigger : config.trigger,
        config
    };
}
...
"[project]/packages/game-core/dist/services/skill-system.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
const combat_utils_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/combat-utils.js [app-client] (ecmascript)");
...
exports.SKILL_HANDLERS = {
    ...
    REDUCE_DAMAGE: function(ctx, skill) {
        let flags = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
...
function applySkills(userSkills, trigger, ctx) {
    ...
    for (const skill of rolled){
        ...
        if (handler) {
            ...
            triggeredSkills.push("".concat(emoji, " **").concat(skill.name, "**"));
        }
...
function getSkillDescription(skill) {
    const chance = "".concat(Math.round((skill.chance || 0.1) * 100), "%");
    let trigger = "";
    ...
    switch(skill.type){
        case "DAMAGE":
            effect = "tăng ".concat(Math.round(skill.multiplier * 100), "% sát thương");
            if (skill.ignoreDef) effect += ", xuyên giáp";
            if (skill.extraHit) effect += ", đánh bồi";
            if (skill.scaleWithHp) effect += ", mạnh hơn khi thấp máu";
            break;
        case "DOT":
            effect = "gây sát thương duy trì (".concat(Math.round(skill.multiplier * 100), "% ATK)");
            break;
        case "HEAL":
            effect = "hồi phục ".concat(Math.round(skill.multiplier * 100), "% ATK lượng máu");
            break;
        case "REDUCE_DAMAGE":
            effect = "giảm sát thương nhận vào ".concat(Math.round(skill.multiplier * 100), "%");
            break;
        case "DODGE":
            effect = "né tránh đòn tấn công";
            break;
        case "SHIELD":
            effect = "tạo khiên ".concat(Math.round(skill.multiplier * 100), "% ATK");
            break;
        default:
            effect = "hiệu ứng đặc biệt";
    }
    return "*".concat(trigger, ", tỉ lệ ").concat(chance, ": ").concat(effect, ".*");
}
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
}),
"[project]/packages/game-core/dist/services/pet-system.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
const skill_system_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/skill-system.js [app-client] (ecmascript)");
const pet_utils_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/pet-utils.js [app-client] (ecmascript)");
    ...
    for (const pet of pets){
        ...
        if (Math.random() <= finalChance) {
            ...
            if (handler) {
                ...
                const logMsg = "".concat(emoji, " **").concat(pet.name, "** thi triển kỹ năng: **").concat(enriched.skillType, "**");
...
"[project]/packages/game-core/dist/services/pet-synergy.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
function getActiveSynergies(pets) {
    ...
    if (pets.length === 3) {
        if (rarities.every((r)=>r === rarities[0])) {
            ...
            synergies.push({
                icon: "✨",
                name: "Three Body Problem (".concat(rarity, ")"),
...
function applyPetSynergy(ctx, pets) {
    ...
    return active.map((syn)=>"".concat(syn.icon, " **").concat(syn.name, "** — ").concat(syn.description));
}
function applyPetPlayerSynergy(ctx, flags) {
    ...
    const add = (title, line)=>{
        names.push(title);
        logs.push("🔗 **".concat(title, "** — ").concat(line));
    };
    if (petCrit && didCrit) {
...
"[project]/packages/game-core/dist/services/stats-service.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
const pet_synergy_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/pet-synergy.js [app-client] (ecmascript)");
const pet_utils_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/pet-utils.js [app-client] (ecmascript)");
const accessory_config_1 = __turbopack_context__.r("[project]/packages/game-core/dist/constants/accessory-config.js [app-client] (ecmascript)");
const titles_1 = __turbopack_context__.r("[project]/packages/game-core/dist/constants/titles.js [app-client] (ecmascript)");
function computeCombatStats(user, items) {
    let pets = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : [], buffs = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : [], petMultipliers = arguments.length > 4 ? arguments[4] : void 0;
    const base = {
        str: user.str,
        agi: user.agi,
        hp: user.maxHp,
    ...
    for (const item of equipped){
        ...
        const bits = [
            wPow ? "WP ".concat(wPow) : null,
            bStr ? "STR+".concat(bStr) : null,
            bAgi ? "AGI+".concat(bAgi) : null,
            bDef ? "DEF+".concat(bDef) : null,
            bHp ? "HP+".concat(bHp) : null
        ].filter(Boolean);
        gearFlatLines.push("".concat(tag, " **").concat(item.name, "** [+").concat(upgradeLevel, "] — ").concat(bits.length ? bits.join(", ") : "không cộng chỉ số phẳng"));
    }
    ...
    if (user.title) {
        ...
        for (const key of equippedKeys){
            ...
            if (!def) continue;
            const tag = "Danh hiệu «".concat(def.name, "»");
            if (def.effectType === "damage") {
                ...
                accessoryEffectLines.push("".concat(tag, ": hệ số ST chí mạng +").concat((def.effectValue * 100).toFixed(0), "% (nền 1.5)"));
            } else if (def.effectType === "burnDamage") {
                multiplier.burnDamage += def.effectValue;
                accessoryEffectLines.push("".concat(tag, ": ST đốt +").concat((def.effectValue * 100).toFixed(0), "%"));
            } else if (def.effectType === "poisonDamage") {
                multiplier.poisonDamage += def.effectValue;
                accessoryEffectLines.push("".concat(tag, ": ST độc +").concat((def.effectValue * 100).toFixed(0), "%"));
            } else if (def.effectType === "lifesteal") {
                multiplier.lifesteal += def.effectValue;
                accessoryEffectLines.push("".concat(tag, ": hút máu +").concat((def.effectValue * 100).toFixed(0), "%"));
            } else if (def.effectType === "procChance") {
                multiplier.procChance += def.effectValue;
                accessoryEffectLines.push("".concat(tag, ": tỉ lệ proc skill +").concat((def.effectValue * 100).toFixed(0), "%"));
            } else if (def.effectType === "goldGain") {
                accessoryEffectLines.push("".concat(tag, ": vàng nhận +").concat((def.effectValue * 100).toFixed(0), "% (ngoài combat)"));
            } else if (def.effectType === "petPower") {
                petTitlePowerMult += def.effectValue;
                accessoryEffectLines.push("".concat(tag, ": +").concat((def.effectValue * 100).toFixed(0), "% vào hệ số sức pet (ATK/DEF pet)"));
            }
        }
    ...
    if (user.talentDps) {
        ...
        damageMultDeltas.push({
            source: "Thiên phú DPS (".concat(user.talentDps, " điểm → +").concat(user.talentDps, "% ST)"),
            delta: d
        });
    }
    if (user.talentTank) {
        ...
        defenseMultDeltas.push({
            source: "Thiên phú Tank (".concat(user.talentTank, " điểm)"),
            delta: d
        });
    }
    if (user.talentSupport) {
        ...
        multiplier.hpMult += d;
        hpMultDeltas.push({
            source: "Thiên phú Support (".concat(user.talentSupport, " điểm → +").concat(user.talentSupport, "% HP)"),
            delta: d
        });
    }
    if (user.talentBurn) {
        ...
        accessoryEffectLines.push("Thiên phú Burn (".concat(user.talentBurn, "): ST đốt +").concat((d * 100).toFixed(0), "%"));
    }
    if (user.talentPoison) {
        const d = user.talentPoison * 0.02;
        multiplier.poisonDamage += d;
        accessoryEffectLines.push("Thiên phú Poison (".concat(user.talentPoison, "): ST độc +").concat((d * 100).toFixed(0), "%"));
    }
    const setCounts = {};
    const applyAccEffect = (eff, upgradeLevel, sourceLabel)=>{
        const scale = 1 + upgradeLevel * 0.05;
        const power = eff.power * scale;
        const pct = (x)=>"".concat((x * 100).toFixed(1), "%");
        if (eff.type === "CRIT_CHANCE") {
            multiplier.critRateBonus += power;
            accessoryEffectLines.push("".concat(sourceLabel, ": tỉ lệ chí mạng +").concat(pct(power)));
        } else if (eff.type === "CRIT_DMG") {
            multiplier.critDamage += power;
            accessoryEffectLines.push("".concat(sourceLabel, ": ST chí mạng (hệ số) +").concat(pct(power)));
        } else if (eff.type === "BURN_DMG") {
            multiplier.burnDamage += power;
            accessoryEffectLines.push("".concat(sourceLabel, ": ST đốt +").concat(pct(power)));
        } else if (eff.type === "POISON_DMG") {
            multiplier.poisonDamage += power;
            accessoryEffectLines.push("".concat(sourceLabel, ": ST độc +").concat(pct(power)));
        } else if (eff.type === "LIFESTEAL") {
            multiplier.lifesteal += power;
            accessoryEffectLines.push("".concat(sourceLabel, ": hút máu +").concat(pct(power)));
        } else if (eff.type === "REDUCE_DMG") {
            multiplier.defenseMult += power;
            defenseMultDeltas.push({
                source: "".concat(sourceLabel, " (giảm ST nhận)"),
                delta: power
            });
        } else if (eff.type === "PROC_CHANCE") {
            multiplier.procChance += power;
            accessoryEffectLines.push("".concat(sourceLabel, ": tỉ lệ kích hoạt skill +").concat(pct(power)));
            ...
            accessoryEffectLines.push("".concat(sourceLabel, ": `").concat(eff.type, "` (power ").concat(power.toFixed(2), ")"));
        } else {
            accessoryEffectLines.push("".concat(sourceLabel, ": ").concat(eff.type, " (").concat(power.toFixed(3), ")"));
        }
    };
    for (const acc of accessories){
        ...
        if (config) {
            ...
            const label = "💍 ".concat(acc.name, " [+").concat(ul, "]");
            config.effects.forEach((e)=>applyAccEffect(e, ul, label));
            if (config.set) setCounts[config.set] = (setCounts[config.set] || 0) + 1;
        } else {
            accessoryEffectLines.push("💍 ".concat(acc.name, ": chưa có trong ACCESSORY_CONFIGS"));
        }
    }
    for (const [setName, count] of Object.entries(setCounts)){
        ...
        if (count >= 2) {
            setBonus.bonus2.forEach((e)=>applyAccEffect(e, 0, "📦 Bộ ".concat(setName, " (≥2 món)")));
            multiplier.activeSets.push(setName);
        }
        if (count >= 3) {
            setBonus.bonus3.forEach((e)=>applyAccEffect(e, 0, "📦 Bộ ".concat(setName, " (≥3 món)")));
        }
    }
    for (const buff of buffs){
        if (buff.type === "STR_PERCENT_BUFF" && buff.power != null) {
            ...
            buffLines.push("Buff STR +".concat(buff.power, "% ST"));
        }
        if (buff.type === "HP_PERCENT_BUFF" && buff.power != null) {
            ...
            buffLines.push("Buff HP +".concat(buff.power, "% máu tối đa"));
        }
    }
    const attackParts = [
        {
            source: "STR nhân vật (".concat(base.str, ") × 0.5"),
            value: base.str * 0.5
        },
        {
            source: "STR trang bị (+".concat(flat.str, ") × 0.5"),
            value: flat.str * 0.5
        },
        {
            source: "AGI nhân vật (".concat(base.agi, ") × 0.3"),
            value: base.agi * 0.3
        },
        {
            source: "AGI trang bị (+".concat(flat.agi, ") × 0.3"),
            value: flat.agi * 0.3
        },
        {
        ...
        {
            source: "Pet (sau danh hiệu pet ×".concat(petTitlePowerMult.toFixed(2), ")"),
            value: flat.petAtk
        }
    ];
    const defenseParts = [
        ...
        {
            source: "DEF từ pet (sau ×".concat(petTitlePowerMult.toFixed(2), ")"),
            value: flat.petDef
        }
    ];
    const hpParts = [
    ...
    const speedParts = [
        {
            source: "SPD = AGI tổng (".concat(totalAgi, ")"),
            value: speed
        }
    ];
    const critChanceParts = [
        {
            source: "LUCK (".concat(base.luck, ") × 0.005 (công thức combat)"),
...
"[project]/packages/game-core/dist/services/relic-system.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
function applyRelicsOnTurn(ctx, relics, turnEvents) {
    for (const r of relics){
        if (r.type === "TURN_HEAL") {
            const heal = r.value;
            ctx.player.hp = Math.min(ctx.player.maxHp, ctx.player.hp + heal);
            turnEvents.push("✨ [Thánh tích] ".concat(r.name, " hồi phục ").concat(heal, " HP."));
            ...
            if (roll < 0.33) {
                ...
                const heal = 15;
                ctx.player.hp = Math.min(ctx.player.maxHp, ctx.player.hp + heal);
                turnEvents.push("🔮 [Chaos Orb] Luồng sáng bí ẩn! (Hồi ".concat(heal, " HP)"));
            } else {
...
function applyRelicsOnKill(state, relics, maxHp) {
    for (const r of relics){
        if (r.type === "ON_KILL_HEAL") {
            ...
            state.floorLogs.push("🧛 [".concat(r.name, "] Hấp thụ sinh khí, hồi ").concat(r.value, " HP! (HP: ").concat(state.hp, "/").concat(maxHp, ")"));
        }
    }
}
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
...
"[project]/packages/game-core/dist/services/relic-engine.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/services/relic-synergy.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
exports.SYNERGY_POOL = [
    {
        ...
        effect: (unit, context)=>{
            if (context.defender) {
                var _context_defender_statusEffects_bleed;
                context.defender.statusEffects = context.defender.statusEffects || {};
                context.defender.statusEffects.bleed = {
                    stacks: (((_context_defender_statusEffects_bleed = context.defender.statusEffects.bleed) === null || _context_defender_statusEffects_bleed === void 0 ? void 0 : _context_defender_statusEffects_bleed.stacks) || 0) + 10
                };
            }
        }
    },
    ...
    },
    {
        ...
        effect: (unit, context)=>{
            if (context.turn === 1) {
                console.log("[Synergy: First Strike] ".concat(unit.id, "'s first attack will critically strike!"));
            }
        }
    },
    {
...
"[project]/packages/game-core/dist/services/combat-engine.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
const skill_system_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/skill-system.js [app-client] (ecmascript)");
const combat_utils_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/combat-utils.js [app-client] (ecmascript)");
const pet_system_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/pet-system.js [app-client] (ecmascript)");
const pet_synergy_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/pet-synergy.js [app-client] (ecmascript)");
const titles_1 = __turbopack_context__.r("[project]/packages/game-core/dist/constants/titles.js [app-client] (ecmascript)");
const relic_system_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/relic-system.js [app-client] (ecmascript)");
async function simulateCombat(options) {
    var _options_accessories, _options_accessories1, _options_accessories2;
    ...
    const ctx = {
        ...
        flags: {
            ...
            enemy: {
                ...
                isBoss: (_enemy_isBoss = enemy.isBoss) !== null && _enemy_isBoss !== void 0 ? _enemy_isBoss : false
            }
        },
        extra: {
            player: {
        ...
        accessories: {
            effects: ((_options_accessories = options.accessories) === null || _options_accessories === void 0 ? void 0 : _options_accessories.effects) || [],
            uniquePowers: ((_options_accessories1 = options.accessories) === null || _options_accessories1 === void 0 ? void 0 : _options_accessories1.uniquePowers) || {},
            sets: ((_options_accessories2 = options.accessories) === null || _options_accessories2 === void 0 ? void 0 : _options_accessories2.sets) || []
        },
        fullLogs: []
    };
    const fullLogs = [];
    ...
    const startLog = "👟 **Tốc độ:** Bạn (**".concat(ctx.player.spd, "**) vs ").concat(ctx.enemy.name, " (**").concat(ctx.enemy.spd, "**). **").concat(firstAttacker, "** ra đòn trước!");
    if (onTurnUpdate) {
        const openLogs = [
            startLog
        ];
    ...
    while(turn <= maxTurns && ctx.player.hp > 0 && ctx.enemy.hp > 0){
        ...
        if (turnStartRes.triggeredSkills.length > 0) {
            turnEvents.push("✨ **Niệm chú:** ".concat(turnStartRes.triggeredSkills.join(", "), "..."));
            if (ctx.extra.player.instantHeal > 0) {
                ...
                turnEvents.push("💚 Hồi phục thêm **".concat(totalHeal, "** HP."));
        ...
        if (turnStartRes.triggeredSynergies.length > 0) {
            turnEvents.push("🔗 **Cộng hưởng (kỹ năng):** ".concat(turnStartRes.triggeredSynergies.join(" · ")));
        ...
        if (!ctx.flags.enemy.bossPhaseTriggered && ctx.enemy.hp <= ctx.enemy.maxHp * 0.5) {
            ...
            turnEvents.push("⚡ **CUỒNG NỘ:** ".concat(ctx.enemy.name, " gồng mình (Tăng ").concat(boostPercent, "% ATK)!"));
        ...
        for (const p of order){
            ...
            if (p.side === 'user') {
                ...
                if (attackRes.triggeredSkills.length > 0) {
                    turnEvents.push("⚔️ **Thi triển:** ".concat(attackRes.triggeredSkills.join(", "), "!"));
                }
                if (attackRes.triggeredSynergies.length > 0) {
                    turnEvents.push("🔗 **Cộng hưởng (kỹ năng):** ".concat(attackRes.triggeredSynergies.join(" · ")));
                }
                ...
                turnEvents.push("🔹 ".concat(skillName, ", gây ").concat(critText, "**").concat(totalBaseDmg, "** sát thương").concat(bonusText, "."));
                ...
                if (ctx.extra.player.bonusDamage > 0) {
                    ...
                    turnEvents.push("✨ **Sát thương kỹ năng cộng thêm:** +**".concat(bonus, "**"));
                ...
                if (totalLifestealPct > 0 && totalTurnDmg > 0) {
                    ...
                    if (heal > 0) {
                        ctx.player.hp = Math.min(ctx.player.maxHp, ctx.player.hp + heal);
                        turnEvents.push("🩸 **Hút máu:** +**".concat(heal, "** HP! (").concat(Math.round(totalLifestealPct * 100), "%)"));
                    }
                }
                ctx.extra.player.bonusDamage = 0;
                turnEvents.push("───");
                ...
                if (defendRes.triggeredSkills.length > 0) {
                    turnEvents.push("🛡️ **Phòng thủ:** ".concat(defendRes.triggeredSkills.join(", "), "..."));
                }
                if (defendRes.triggeredSynergies.length > 0) {
                    turnEvents.push("🔗 **Cộng hưởng (kỹ năng):** ".concat(defendRes.triggeredSynergies.join(" · ")));
                ...
                if (ctx.flags.player.dodged || defendRes.flags.didDodge) {
                    turnEvents.push("> 💨 **".concat(enemy.name, "** ra chiêu nhưng bạn đã né được!"));
                } else {
                    ...
                    turnEvents.push("> 🔸 **".concat(enemy.name, "** tấn công, gây **").concat(finalDmg, "** sát thương."));
                    ...
                    if (hasReflect) {
                        ...
                        if (reflected > 0) {
                            ...
                            turnEvents.push("> 🦔 **Nhím Châu Âu** phản lại **".concat(reflected, "** sát thương!"));
                        }
                    }
                }
                turnEvents.push("───");
...
"[project]/packages/game-core/dist/services/upgrade-service.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/services/shop-service.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
function rollShopItemsFn() {
    let excludeKeys = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : new Set();
    ...
    for(let slot = 0; slot < SHOP_SLOTS; slot++){
        const r = Math.random();
        var _pools_;
        let pool = (_pools_ = pools[0]) !== null && _pools_ !== void 0 ? _pools_ : [];
        var _pools_1;
        if (r < 0.25) pool = (_pools_1 = pools[1]) !== null && _pools_1 !== void 0 ? _pools_1 : [];
        var _pools_2;
        if (r < 0.12) pool = (_pools_2 = pools[2]) !== null && _pools_2 !== void 0 ? _pools_2 : [];
        let chosen;
...
/** Returns 5 daily skills deterministically based on userId + VN date. */ function getDailySkills(userId, skills, getVnDayString) {
    const seedString = "".concat(userId, "-").concat(getVnDayString());
    let hash = 0;
...
"[project]/packages/game-core/dist/services/quest-service.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
"[project]/packages/game-core/dist/utils/time.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
function getVnDayString() {
    let date = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : new Date();
    var _getVnDate_toISOString_split_;
    return (_getVnDate_toISOString_split_ = getVnDate(date).toISOString().split("T")[0]) !== null && _getVnDate_toISOString_split_ !== void 0 ? _getVnDate_toISOString_split_ : "";
}
...
"[project]/packages/game-core/dist/services/index.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
var rng_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/rng.js [app-client] (ecmascript)");
...
var combat_utils_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/combat-utils.js [app-client] (ecmascript)");
...
var leveling_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/leveling.js [app-client] (ecmascript)");
...
var hunt_service_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/hunt-service.js [app-client] (ecmascript)");
...
var pet_utils_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/pet-utils.js [app-client] (ecmascript)");
...
var pet_system_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/pet-system.js [app-client] (ecmascript)");
...
var pet_synergy_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/pet-synergy.js [app-client] (ecmascript)");
...
var skill_system_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/skill-system.js [app-client] (ecmascript)");
...
var stats_service_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/stats-service.js [app-client] (ecmascript)");
...
var relic_system_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/relic-system.js [app-client] (ecmascript)");
...
var relic_engine_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/relic-engine.js [app-client] (ecmascript)");
...
var relic_synergy_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/relic-synergy.js [app-client] (ecmascript)");
...
var combat_engine_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/combat-engine.js [app-client] (ecmascript)");
...
var upgrade_service_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/upgrade-service.js [app-client] (ecmascript)");
...
var shop_service_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/shop-service.js [app-client] (ecmascript)");
...
var quest_service_1 = __turbopack_context__.r("[project]/packages/game-core/dist/services/quest-service.js [app-client] (ecmascript)");
...
var time_1 = __turbopack_context__.r("[project]/packages/game-core/dist/utils/time.js [app-client] (ecmascript)");
...
"[project]/packages/game-core/dist/index.js [app-client] (ecmascript)", ((__turbopack_context__, module, exports) => {
"use strict";
...
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/types/index.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/constants/index.js [app-client] (ecmascript)"), exports);
__exportStar(__turbopack_context__.r("[project]/packages/game-core/dist/services/index.js [app-client] (ecmascript)"), exports);
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(__turbopack_context__.m, globalThis.$RefreshHelpers$);
}
}),
...
//# sourceMappingURL=packages_game-core_dist_3bb49fc9._.js.map