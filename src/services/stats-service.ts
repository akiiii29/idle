...
import { ACCESSORY_CONFIGS, ACCESSORY_SETS, AccessoryEffect } from "../constants/accessory-config";
...
/** Breakdown for `/stats` — nguồn gốc từng thành phần (khớp công thức combat). */
...
export function computeCombatStats(
    ...
    // 2. Flat Stats (Equipment & Pets)
    const flat: FlatStats = { str: 0, agi: 0, def: 0, hp: 0, weaponPower: 0, petAtk: 0, petDef: 0 };
    
    const itemsWithLevel = (items as any[]) as { name: string; type: string | ItemType; power: number; bonusStr?: number; bonusAgi?: number; bonusDef?: number; bonusHp?: number; isEquipped?: boolean; set?: string | null; upgradeLevel?: number }[];
    ...
    for (const item of equipped) {
        ...
        const scale = 1 + (upgradeLevel * 0.1); // +10% per level for Weapon/Armor

        if (item.type === "ACCESSORY" || item.type === ItemType.ACCESSORY) {
        ...
        flat.hp += bHp;
        let wPow = 0;
        if (item.type === ItemType.WEAPON || item.type === "WEAPON") {
            wPow = Math.floor((item.power || 0) * scale);
            flat.weaponPower += wPow;
        }
        const tag = item.type === ItemType.WEAPON || item.type === "WEAPON" ? "⚔️" : "🛡️";
        const bits = [
            wPow ? `WP ${wPow}` : null,
            bStr ? `STR+${bStr}` : null,
            bAgi ? `AGI+${bAgi}` : null,
    ...
    // 4. Title Bonuses (Process first to update flat.petAtk and multipliers)
    if (user.title) {
        ...
        try {
            if (user.title.startsWith("[")) {
                ...
                equippedKeys = [user.title]; // Legacy support
            }
        } catch (e) {}
    ...
    // 5. Derived Stats (Now uses title-buffed pet stats)
    ...
    // 6. Other Multipliers (Pet / Buff / Talent / Accessory)
...
// Global Damage Calculation helper using FINAL stats ONLY
