 ...
 * Finalized by Senior Combat Systems Engineer.
 */
...
export async function simulateCombat(options: {
  ...
  if (options.player.title) {
    ...
    if (titleDef) {
       ...
       else if (type === "petPower") ctx.player.petPower += val; // Not really supported simply, but okay
    }
  }
  ...
  // UI summary accumulators (aggregated across all turns for final result)
  const uiAccum = {
  ...
  if (options.relics && options.relics.length > 0) {
    // Relic logic inside combat engine requires the function
    const { applyRelicsBeforeCombat } = require("./relic-system");
    applyRelicsBeforeCombat(ctx, options.relics);
  }

  let turn = 1;

  // --- 3. DETERMINE TURN ORDER (Once at start as requested) ---
  ...
  // Add as a pseudo-turn 0 or inject into first update
  if (onTurnUpdate) {
  ...
  while (turn <= maxTurns && ctx.player.hp > 0 && ctx.enemy.hp > 0) {
    ...
    if (turnStartRes.triggeredSkills.length > 0) {
      ...
      // Apply immediate healing/bonus damage from start of turn skills
      ...
      if (ctx.extra.player.bonusDamage > 0) {
        const { applyDamage } = require("./combat-utils");
    ...
    // --- 4. EXECUTE ACTIONS (Goal 8-4) ---
    for (const p of order) {
      if (ctx.player.hp <= 0 || ctx.enemy.hp <= 0) break;

      // Reset turn-specific flags and multipliers
      ...
      if (p.side === 'user') {
        const { calculateDamage, applyDamage } = require("./combat-utils");
        ...
        // Accumulate UI summary: skill activation counts
        ...
        // --- NATURAL CRIT ROLL (Moved earlier for synergy support) ---
        ...
        // 2. Log and Apply Bonus Damage (Specialized skills)
        if (ctx.extra.player.bonusDamage > 0) {
        ...
        ctx.extra.player.bonusDamage = 0; // consumed
        turnEvents.push("───"); // Separator after user action
      } else {
        const { calculateDamage, applyDamage, getBuffDamageReduction } = require("./combat-utils");
        ...
        if (ctx.flags.player.dodged || defendRes.flags.didDodge) {
          ...
          // UNIQUE_BLOCK_HIT check (Simplification: block once every 3 turns if available)
    ...
    // --- 6. CLEANUP EXPIRED EFFECTS (Goal 8-6) ---
