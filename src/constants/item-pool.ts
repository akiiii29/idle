...
export function rollItemRarity(floor: number, luckBonus: number = 0): ItemRarity {
    const r = (Math.random() * 100) - luckBonus; // Lower roll = better loot
    const epicChance      = Math.min(30, floor * 3);       // +3% per floor, cap 30
    const legendaryChance = Math.min(10, floor * 0.8);     // +0.8% per floor, cap 10
    const rareChance      = Math.min(40, floor * 5);       // +5% per floor, cap 40
...
export function rollLootDrop(floor: number, luckBonus: number = 0): GameItem | undefined {
    ...
    // Fallback to COMMON if no items at rarity level
